...
const LABEL_VALUE = 'vitest_localstack'
...
const isVitestRuning = process.env.VITEST_WORKER_ID
...
async function factoryConfig() {
  const pathConfig = path.resolve(cwd(), 'localstack.config.js')
...
async function createKinesisStreams(config, services) {
  if (Array.isArray(config.Kinesis) && services.kinesis) {
    const kinesis = new Kinesis({
      endpoint: `http://localhost:${services.kinesis || DEFAULT_PORT}`,
      region: 'us-east-1',
    })
...
async function createS3Buckets(config, services) {
  if (Array.isArray(config.S3Buckets) && services.s3) {
    const s3 = new AWS.S3({
      endpoint: `http://localhost:${services.s3 || DEFAULT_PORT}`,
...
module.exports.isVitestRuning = isVitestRuning
